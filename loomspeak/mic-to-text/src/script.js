class MicToMP3Transcriber {
  constructor() {
    ...
    // Parsed actions UI
    this.parsedActionsSection = document.getElementById("parsedActionsSection");
    this.parsedActionsResult = document.getElementById("parsedActionsResult");
  ...
  setupEventListeners() {
    ...
    this.transcribeUploadBtn.addEventListener("click", () =>
  ...
  async transcribeAudio() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Also call the parser
        await this.parseActionsFromText(result.transcript);
  ...
  async parseActionsFromText(text) {
    if (!text || typeof text !== "string" || !text.trim()) {
      return;
    }
    try {
      this.updateStatus("Parsing actions with Gemini...", "processing");
      // Assume model-output server is running on 3010 by default
      const baseUrl = window.MODEL_OUTPUT_API || "http://localhost:3010";
      ...
      if (!res.ok) {
        ...
        throw new Error(`Parser error ${res.status}: ${errText.slice(0, 200)}`);
      }
      const data = await res.json();
      const pretty = JSON.stringify(data, null, 2);
      this.parsedActionsResult.textContent = pretty;
      this.parsedActionsSection.style.display = "block";
      this.parsedActionsSection.classList.remove("hidden");
      // Open the model-output page in a new tab so the user can view the rich summary
      try {
        const viewUrl = `${baseUrl}/`;
        window.open(viewUrl, "_blank", "noopener,noreferrer");
      } catch (openErr) {
        console.warn(
          "Could not open model-output page automatically:",
          openErr
        );
      }
    } catch (e) {
      console.error("Parsing error", e);
      this.parsedActionsResult.textContent = `Error parsing actions: ${e.message}`;
      this.parsedActionsSection.style.display = "block";
      this.parsedActionsSection.classList.remove("hidden");
    }
  }
