...
app.delete('/recordings/:filename', (req, res) => {
  try {
    ...
    if (!filePath.startsWith(path.resolve('./recordings'))) {
...
app.listen(PORT, () => {
  console.log(`Mic-to-MP3 + Transcription server running at http://localhost:${PORT}`);
  console.log(`Recordings will be saved to: ${path.resolve('./recordings')}`);
  console.log(`Voice-to-text processor: ${path.resolve('../voice-to-text/processor.js')}`);
});
